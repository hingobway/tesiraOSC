...
// const TESIRA_READY = 'Welcome to the Tesira Text Protocol Server...\r\n';
const TESIRA_READY = '\r\n';
...
try {
  ...
  c.on('data', (d: Buffer) => {
    if (sent) return;
    if (!d.toString().includes(TESIRA_READY)) return;

    const cmd = (a: number, b: number) =>
      `Mixer1 set crosspointLevelState ${a} ${b} true`;
